...
const Collections = () => {
  ...
  const [sortType,setShortType] = useState("ralevent")
  ...
  const applyFilter = () =>{
      ...
      if(category.length > 0) {
        productCopy = productCopy.filter(items => category.includes(items.category.toLowerCase()))
      } 
  ...
  return (
    <div className='w-[99vw] min-h-[100vh] bg-gradient-to-l from-[#141414]
    to-[#0c2025] flex items-start flex-col md:flex-row justify-start pt-[70px]
    overflow-x-hidden z-[2] pb-[110px]'  >
        <div className={`md:w-[30vw] lg:w-[20vw] w-[100vw] md:min-h-[100vh] 
        ${showFilter ? "h-[50vh]" : "h-[8vh]"} p-[20px] 
        ...
        <div className='lg:pl-[20%] md:py-[10px]'>
              <div className='md:w-[80vw] w-[100vw] p-[20px] flex justify-between flex-col
              lg:flex-row lg:px-[50px]'>
                  <Title text1={"ALL"} text2={"COLLECTIONS"}/>

                  <select name="" id="" className='bg-slate-600 w-[60%] md:w-[200px] h-[50px] px-[10px] text-white 
                  rounded-lg hover:border-[#46d1f7] border-[2px]' onChange={(e)=>setShortType(e.target.value)}>
                    <option value="relevant" className='w-[100%] h-[100%]'>
                        Sort By: Relevant </option>
            
                    <option value="low-high">
                       Sort By: Low-high</option>
                    
                    <option value="high-low">
                     Sort By: High-low  </option>
                  </select>
              </div>
